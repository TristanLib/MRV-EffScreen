...
\newcommand{\reviewversion}{Internal review version 0.3.2, 20 May 2026}
\makeatletter
...
This study addresses three research questions: whether non-leakage public MRV features can support moderate year-ahead relative CO$_2$-intensity stratification; how much random stratified evaluation overstates performance relative to temporal evaluation; and whether related emission, fuel, and time fields can support a cautious manual consistency-review queue without being interpreted as regulatory findings.
...
\item A separate consistency-screening module combines Isolation Forest, Local Outlier Factor, and regression residual ranking to prioritize manual consistency-review candidates.
\end{enumerate}
...
\subsection{Leakage Control and Consistency-Review Screening}
...
The main target is a three-class relative efficiency label derived from the CO$_2$-per-distance variable. Within each \texttt{ship\_type} and \texttt{reporting\_year} group, records are ranked by CO$_2$ emissions per distance and divided into tertiles: \texttt{efficient}, \texttt{medium}, and \texttt{inefficient}. Labels are assigned only when the corresponding ship-type-year group contains at least 30 records with non-missing CO$_2$ per distance. Groups below this threshold and rows with missing target values remain unlabeled and are excluded from the classification dataset. In the 2018--2023 main experiment, 73,045 of 75,580 records are labeled; the remaining 2,535 records comprise 69 records from below-threshold groups and 2,466 records with missing target values. This construction avoids direct comparison of raw CO$_2$ intensity across fundamentally different ship categories and controls for annual shifts in fleet composition and reporting conditions. The label is descriptive and relative within the MRV dataset; it is not CII, EEXI, a certified energy-efficiency score, or a regulatory compliance label.
...
The temporal design does not remove all repeated-entity dependence. An IMO-overlap audit shows that 11,457 of 13,050 unique IMO numbers in the 2022 holdout also appear in the 2018--2021 training split, as do 10,439 of 12,615 in the 2023 test split and 10,029 of 12,905 in the 2024 external split (Table~\ref{tab:imo_overlap}). Thus, the primary result is best interpreted as year-ahead repeated-fleet generalization. A stricter new-vessel generalization test would require grouped-by-IMO evaluation.

\begin{table}[H]
\caption{IMO-number recurrence between the 2018--2021 training split and later-year evaluation splits.\label{tab:imo_overlap}}
\begin{tabularx}{\textwidth}{LCCC}
\toprule
\textbf{Later Split} & \textbf{Later Unique IMO} & \textbf{Overlap IMO} & \textbf{Overlap Fraction}\\
\midrule
2022 holdout & 13,050 & 11,457 & 0.878\\
2023 test & 12,615 & 10,439 & 0.828\\
2024 external & 12,905 & 10,029 & 0.777\\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Consistency-Review Screening and Sensitivity Analysis}
...
\begin{table}[H]
\caption{Sensitivity of consistency-review candidates under 1\%, 2\%, and 5\% ship-type-specific thresholds.\label{tab:sensitivity}}
\footnotesize
\begin{tabularx}{\textwidth}{LCCCCCCC}
\toprule
\textbf{Thresh.} & \textbf{All Three} & \textbf{At Least Two} & \textbf{Any Method} & \textbf{Top 200 At Least Two} & \textbf{Top 200 All Three} & \textbf{Jaccard At Least Two} & \textbf{Jaccard All Three}\\
\midrule
1\% & 97 & 487 & 2,011 & 189 & 97 & 0.456 & 0.429\\
2\% & 226 & 1,067 & 3,888 & 200 & 182 & 1.000 & 1.000\\
5\% & 643 & 2,626 & 9,640 & 200 & 200 & 0.406 & 0.351\\
\bottomrule
\end{tabularx}
\normalsize
\end{table}
...
\caption{Ship-type distribution of the top 200 consistency-review candidates.\label{fig:anomaly_ship_types}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/mrv_anomaly_sensitivity_overlap.pdf}
\caption{Sensitivity of consistency-review overlap under 1\%, 2\%, and 5\% thresholds.\label{fig:sensitivity}}
\end{figure}
...
\bibitem{roberts2017cross_validation_structured}
Roberts, D.R.; Bahn, V.; Ciuti, S.; Boyce, M.S.; Elith, J.; Guillera-Arroita, G.; Hauenstein, S.; Lahoz-Monfort, J.J.; Schroeder, B.; Thuiller, W.; Warton, D.I.; Wintle, B.A.; Hartig, F.; Dormann, C.F. Cross-validation strategies for data with temporal, spatial, hierarchical, or phylogenetic structure. \textit{Ecography} \textbf{2017}, \textit{40}, 913--929. \url{https://doi.org/10.1111/ecog.02881}.

\bibitem{bergmeir2018time_series_cv}
Bergmeir, C.; Hyndman, R.J.; Koo, B. A note on the validity of cross-validation for evaluating autoregressive time series prediction. \textit{Comput. Stat. Data Anal.} \textbf{2018}, \textit{120}, 70--83. \url{https://doi.org/10.1016/j.csda.2017.11.003}.
