...
本文档定义建模时使用的特征集，目的是避免标签泄漏，并把“能效分类”和“一致性复核候选筛查”两类任务分开。
...
只有 `ship_type + reporting_year` 组内 `co2_per_distance_kg_nm` 非缺失样本数不少于 30 行时才分配三分位标签；小于 30 行的组和目标字段缺失的行保留为未标注记录，不进入主分类实验。
...
- `train`: 2018-2021
- `holdout_2022`: 2022，仅作为报告型时间 holdout，不用于调参或模型选择
- `test_2023`: 2023，主时间外推测试集
...
用途：一致性复核候选筛查、数据质量案例分析。不要用于主分类结论。
...
- 论文中必须写明它用于人工一致性复核候选排序，不用于证明预测能力，也不构成监管结论。
